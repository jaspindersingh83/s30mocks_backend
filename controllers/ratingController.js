...
exports.createRating = async (req, res) => {
  try {
    ...
    if (interview.candidate.toString() !== req.user.id) {
      return res.status(403).json({ message: 'You can only rate interviews you participated in as a candidate' });
    }
    
    // Verify the interview is completed
    if (interview.status !== 'completed') {
      return res.status(400).json({ message: 'You can only rate completed interviews' });
    }
    ...
    if (existingRating) {
      return res.status(400).json({ message: 'You have already rated this interview' });
    }
