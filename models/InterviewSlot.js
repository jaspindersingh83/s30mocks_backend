...
InterviewSlotSchema.pre('save', function(next) {
  ...
  if (this.interviewType === 'DSA') {
    ...
    if (Math.abs(duration - 40) > 1) { // Allow 1 minute tolerance
    ...
    if (Math.abs(duration - 50) > 1) { // Allow 1 minute tolerance
      const error = new Error('System Design interview slots must be 50 minutes');
      return next(error);
    }
  }
